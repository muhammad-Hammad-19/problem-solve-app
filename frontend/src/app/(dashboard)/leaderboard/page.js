...
const LeaderboardPage = () => {
  ...
  useEffect(() => {
    const fetchLeaderUser = async () => {
      try {
        const res = await axios.get("http://localhost:5000/request/fetch", {
        ...
        if (res.data.successful && res.data.helperCounter) {
          ...
          const sortedUsers = formattedUsers.sort((a, b) => b.points - a.points);
  ...
  const renderRankBadge = (rank) => {
    if (rank === 1) return <Trophy className="text-amber-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.4)]" size={24} />;
  ...
  const renderStatus = (status) => {
    if (status === "up") return <ArrowUp className="text-green-500" size={16} />;
    if (status === "down") return <ArrowDown className="text-red-500" size={16} />;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-800 pb-6">
          <div>
            <h1 className="text-3xl md:text-4xl font-black bg-gradient-to-br from-white to-zinc-500 bg-clip-text text-transparent flex items-center gap-3">
              <Trophy className="text-indigo-500" size={36} /> Global Leaderboard
            </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end pt-4">
            ...
            {filteredUsers[1] && (
              <div className="bg-zinc-900/30 border border-zinc-800/80 p-6 rounded-[2rem] text-center order-2 sm:order-1 sm:h-[180px] flex flex-col justify-center items-center relative group hover:border-zinc-700 transition-all">
                ...
                <h3 className="font-bold text-zinc-200">{filteredUsers[1].name}</h3>
                <p className="text-xs text-zinc-500">{filteredUsers[1].points} pts</p>
                <div className="absolute top-4 left-4 bg-zinc-800 px-2 py-0.5 rounded-md text-xs text-zinc-400 font-bold">#2</div>
              </div>
            )}
            ...
            {filteredUsers[0] && (
              <div className="bg-zinc-900/60 border border-indigo-500/30 p-6 rounded-[2rem] text-center order-1 sm:order-2 sm:h-[210px] flex flex-col justify-center items-center relative shadow-lg shadow-indigo-500/5 group hover:border-indigo-500/50 transition-all">
                ...
                <h3 className="font-extrabold text-white text-lg">{filteredUsers[0].name}</h3>
                <p className="text-sm text-indigo-400 font-bold">{filteredUsers[0].points} pts</p>
                <div className="absolute top-4 left-4 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-md text-xs text-indigo-400 font-bold">#1</div>
              </div>
            )}
            ...
            {filteredUsers[2] && (
              <div className="bg-zinc-900/30 border border-zinc-800/80 p-6 rounded-[2rem] text-center order-3 sm:h-[160px] flex flex-col justify-center items-center relative group hover:border-zinc-700 transition-all">
                ...
                <h3 className="font-bold text-zinc-300">{filteredUsers[2].name}</h3>
                <p className="text-xs text-zinc-500">{filteredUsers[2].points} pts</p>
                <div className="absolute top-4 left-4 bg-zinc-800 px-2 py-0.5 rounded-md text-xs text-zinc-500 font-bold">#3</div>
              </div>
            )}
          </div>
        )}
